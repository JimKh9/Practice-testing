import socket

# Список портов для сканирования
ports = [20, 21, 22, 23, 25, 42, 43, 53, 67, 69, 80, 110, 115, 123, 137, 138, 139, 143, 161, 179, 443, 445, 514, 515, 993, 995, 1080, 1194, 1433, 1702, 1723, 3128, 3268, 3306, 3389, 5432, 5060, 5900, 5938, 8080, 10000, 20000]
host = input('Введите имя сайта без http/https или IP-адрес: ')
print('Ожидай, идет сканирование портов!')

# В цикле перебираем порты из списка
for port in ports:
    s = socket.socket()                    # Создаем сокет
    s.settimeout(1)                        # Устанавливаем тайм-аут в одну cекунду
    try:
        s.connect((host, port))            # Пробуем соединиться, хост и порт передаем как список
    except socket.error:                   # Если соединение вызвало ошибку
        pass
else:
    print(f'{host}: {port} порт активен')
    s.close()                              # Закрываем соединение